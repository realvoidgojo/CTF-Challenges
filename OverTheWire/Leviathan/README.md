...
Save in file and name as leviathan0
...
last parameter take to the third parameter using `%s` format specifier , and then we can blitz like cmd line execution in line 6 `system()` call and make temp directory `mktemp -d`
...
rm rand bash
```
touch will create two files like rand;bash and also open sub shell inside terminal and `exit`
so using `'rand;bash'` will create single file as `rand;bash`
The tilde `~` is a Linux "shortcut" to denote a user's home directory
```bash
~/printfile 'rand;bash'
/bin/cat: rand: Permission denied
leviathan3@gibson:/tmp/tmp.xyIIGjCrrh$
...
./bin | tr " " "\n"
```
`bc` is and CLI calculator , output_base = 16 , input_base = 2  16,2 are number systems
```bash
